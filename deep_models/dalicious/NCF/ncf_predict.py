...
def predict_ncf(num_users, num_items, embedding_size, layers, reg_layers, reg_mf, 
    ...
    new_model = NCF(num_users, num_items, embedding_size, layers, reg_layers, reg_mf)
    ...
    output_input = [np.array(pred_users), np.array(pred_foods)]
    newpred = new_model.predict(output_input)
